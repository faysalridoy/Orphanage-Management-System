...
<style>
	*{
	...
	#userhero{
		height: 100%;
		width: 100%;
		background-image: linear-gradient(rgba(0,0,0,0.4),rgba(0,0,0,0.4)),url(./img/userreg.png);
...
if(isset($_POST['submit'])){
	if(!empty($_POST['userid']) && ($_POST['password'])){

    $adminid = $_POST['userid'];
    $password = $_POST['password'];
    $query= "SELECT * FROM user WHERE userid='{$adminid}' AND  password='{$password}'" ;
    ...
    if($number_of_user > 0){
       ...
       echo '<b><div style="font-size:1.25em;color:red;padding-top:250px;padding-left:650px">*No user found..!Please register first.</div></b>';
    }
   }else{
